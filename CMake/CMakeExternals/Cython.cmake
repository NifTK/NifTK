...
if( MITK_USE_Python AND NOT MITK_USE_SYSTEM_PYTHON )
  ...
  # Sanity checks
  if(DEFINED Cython_DIR AND NOT EXISTS ${Cython_DIR})
    message(FATAL_ERROR "Cython_DIR variable is defined but corresponds to non-existing directory")
  endif()

  set(version "0.23.3")
  set(location "${NIFTK_EP_TARBALL_LOCATION}/Cython-${version}.tar.gz")
  niftkMacroDefineExternalProjectVariables(Cython ${version} ${location})
  set(proj_DEPENDENCIES Python)

  if( NOT DEFINED Cython_DIR )
    ...
    set(_cython_build_step ${CMAKE_SOURCE_DIR}/CMake/mitkFunctionExternalPythonBuildStep.cmake)

    set(_configure_step ${proj_CONFIG}/${proj}_configure_step.cmake)
    file(WRITE ${_configure_step}
       "${_cython_env}
        include(\"${_cython_build_step}\")
    ...
    set(_cython_compiler )
    if(WIN32)
     set(_cython_compiler --compiler=msvc)
    endif()
    ...
    file(WRITE ${_build_step}
       "${_cython_env}
        include(\"${_cython_build_step}\")
        mitkFunctionExternalPythonBuildStep(${proj} build \"${PYTHON_EXECUTABLE}\" \"${CMAKE_BINARY_DIR}\" setup.py build ${_cython_compiler})
       ")
    ...
    set(Cython_DIR ${MITK_PYTHON_SITE_DIR}/cython)
    install(SCRIPT ${_install_step})

    message("SuperBuild loading Cython from ${Cython_DIR}")

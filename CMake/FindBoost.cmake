# NifTK-Modified: This is a modified version of the one that comes with CMake 2.6-patch 4
# NifTK-Modified: Search for string NifTK-Modified to see what was changed.
...
SET( _boost_TEST_VERSIONS ${Boost_ADDITIONAL_VERSIONS} "1.47.0" "1.46.1" "1.46.2" "1.44.0" "1.36.1" "1.36.0" "1.35.1" "1.35.0" "1.35" "1.34.1" "1.34.0" "1.34" "1.33.1" "1.33.0" "1.33" )
...
ELSE (_boost_IN_CACHE)
  ...
  # NifTK-Modified
  # MESSAGE("Boost compiler1 ${_boost_COMPILER}")
  ...
  IF (UNIX)
    ...
    ELSE (APPLE)
      ...
      ELSE (NOT CMAKE_COMPILER_IS_GNUCC)
        ...
        # NifTK-Modified: This is the full compiler version info string, we want the first two numbers ie. 41 from 4.1.2.
        # MESSAGE("Boost compiler2 ${_boost_COMPILER_VERSION}")

# NifTK-Modified: This assumes you have a space after the last version number, we dont, we have a newline        
#        STRING(REGEX REPLACE ".* ([0-9])\\.([0-9])\\.[0-9] .*" "\\1\\2"
#               _boost_COMPILER_VERSION ${_boost_COMPILER_VERSION})

# NifTK-Modified: So we use this one instead.
        STRING(REGEX REPLACE ".* ([0-9])\\.([0-9])\\.[0-9].*" "\\1\\2"
               _boost_COMPILER_VERSION ${_boost_COMPILER_VERSION})
        ...
        #MESSAGE("Boost compiler3 ${_boost_COMPILER_VERSION}")
        #MESSAGE("Boost compiler4 ${_boost_COMPILER}")
...
# NifTK-Modified: Yes, i was debugging this.
#
   MESSAGE("Search paths:${_boost_LIBRARIES_SEARCH_DIRS}")
   MESSAGE("Finding ${COMPONENT}")
   MESSAGE("Prefix: ${Boost_LIB_PREFIX}")
   MESSAGE("Compiler: ${_boost_COMPILER}")
   MESSAGE("Multi: ${_boost_MULTITHREADED}")
   MESSAGE("static tag: ${_boost_STATIC_TAG}")
   MESSAGE("version: ${Boost_LIB_VERSION}")
   MESSAGE("Search paths:     ${_boost_LIBRARIES_SEARCH_DIRS}")
   MESSAGE("Names: ${Boost_LIB_PREFIX}boost_${COMPONENT}${_boost_COMPILER}${_boost_MULTITHREADED}-${Boost_LIB_VERSION}")
   MESSAGE("Names: ${Boost_LIB_PREFIX}boost_${COMPONENT}${_boost_COMPILER}${_boost_MULTITHREADED}${_boost_STATIC_TAG}-${Boost_LIB_VERSION}")
   MESSAGE("Names: ${Boost_LIB_PREFIX}boost_${COMPONENT}${_boost_MULTITHREADED}")
   MESSAGE("Names: ${Boost_LIB_PREFIX}boost_${COMPONENT}${_boost_MULTITHREADED}${_boost_STATIC_TAG}")
   MESSAGE("Names: ${Boost_LIB_PREFIX}boost_${COMPONENT}")

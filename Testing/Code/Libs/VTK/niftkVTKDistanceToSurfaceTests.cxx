...
int niftkVTKDistanceToSurfaceTest ( int argc, char * argv[] )
{
  if ( argc != 4 )
  {
    std::cerr << "Usage niftkVTKDistanceToSurfaceTest source target groundtruth" << std::endl;
    return EXIT_FAILURE;
  }
...
int niftkVTKDistanceToSurfaceTestSinglePoint ( int argc, char * argv[] )
{
  if ( argc != 6 )
  {
    std::cerr << "Usage niftkVTKDistanceToSurfaceTestSinglePoint x y z target groundtruth" << std::endl;
    return EXIT_FAILURE;
  }
  ...
  double distance = niftk::DistanceToSurface (p, target);

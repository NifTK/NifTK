...
#ifndef niftkImageUtils_h
#define niftkImageUtils_h
...
/// \file niftkImageUtils.h
/// \brief Some useful MIDAS related image utilities, such as working out the As Acquired orientation, volumes etc.
namespace niftk
{

/// \brief ITK method that given an image, returns the ImageOrientation for the XY plane.
/// \param itkImage an ITK image
/// \param outputOrientation the output ImageOrientation as either IMAGE_ORIENTATION_AXIAL,
/// IMAGE_ORIENTATION_CORONAL or IMAGE_ORIENTATION_SAGITTAL, or else is unchanged from the input.
...
/// \brief Returns the ImageOrientation corresponding to the XY plane, or else returns the supplied default.
/// \param defaultOrientation A default ImageOrientation that will be returned if we can't work out the As Acquired orientation.
/// \param image An image to check.
/// \return ImageOrientation the As Acquired orientation, or the defaultOrientation.
NIFTKCORE_EXPORT ImageOrientation GetAsAcquiredOrientation(ImageOrientation defaultOrientation, const mitk::Image* image);

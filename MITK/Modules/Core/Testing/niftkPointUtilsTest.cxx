...
void TestCalculateStepSize()
{
  ...
  spacing[1] = 0.0;
  spacing[2] = 0.0;
  result = CalculateStepSize(spacing);
  ...
  spacing[2] = 1.0;
  result = CalculateStepSize(spacing);
  MITK_TEST_CONDITION_REQUIRED(mitk::Equal(result, 1.0 / 3.0),".. Testing result==1/3");

  spacing[0] = 0.5;
  spacing[1] = 1.0;
  spacing[2] = 1.0;
  result = CalculateStepSize(spacing);
  MITK_TEST_CONDITION_REQUIRED(mitk::Equal(result, 1.0 / 6.0),".. Testing result==1/6");

  spacing[0] = 1.0;
  spacing[1] = 1.0;
  spacing[2] = 1.0 / 3.0;
  result = CalculateStepSize(spacing);
  MITK_TEST_CONDITION_REQUIRED(mitk::Equal(result, 1.0 / 9.0),".. Testing result==1/9");
  ...
  for (unsigned int i = 0; i < 3; i++)
  {
    for (int j = -1; j<=1; j++)
    {
      ...
      float distance =  (a[0]-b[0])*(a[0]-b[0])
                      + (a[1]-b[1])*(a[1]-b[1])
                      + (a[2]-b[2])*(a[2]-b[2]);
      float result = GetSquaredDistanceBetweenPoints(a,b);
...
void TestFindLargestDistanceBetweenTwoPoints()
{
  ...
  double distance = FindLargestDistanceBetweenTwoPoints(*pointSet);
...
void TestScalePointSets()
{
  ...
  ScalePointSets(*inputPointSet, *outputPointSet, 1);
  MITK_TEST_CONDITION_REQUIRED(!AreDifferent(inputPointSet->GetPoint(0), outputPointSet->GetPoint(0)),".. Testing that scale factor 1 does nothing. " );
  ScalePointSets(*inputPointSet, *outputPointSet, 2);
  double squaredDistance = GetSquaredDistanceBetweenPoints(inputPointSet->GetPoint(0), outputPointSet->GetPoint(0));
  ...
  ScalePointSets(*inputPointSet, *outputPointSet, 3);
  squaredDistance = GetSquaredDistanceBetweenPoints(inputPointSet->GetPoint(0), outputPointSet->GetPoint(0));
...
void TestComputeNormalFromPoints()
{
  ...
  c[1] = 2;
  c[2] = 0;
  ComputeNormalFromPoints(a, b, c, output);
...
void TestFilterMatchingPoints()
{
  ...
  int matchedPoints = FilterMatchingPoints(*fixedPoints, *movingPoints, *outputFixedPoints, *outputMovingPoints);
...
void TestRemoveNaNPoints()
{
  ...
  int naNRemovedPoints = RemoveNaNPoints(*naNPoints, *outputNaNPoints);
  int noNaNRemovedPoints = RemoveNaNPoints(*noNaNPoints, *outputNoNaNPoints);
...
void TestCheckForNaNPoint()
{
  ...
  MITK_TEST_CONDITION_REQUIRED(mitk::Equal(CheckForNaNPoint(p1), false),".. Testing that CheckFormNaNPoint returns false for a point with no NaNs");

  MITK_TEST_CONDITION_REQUIRED(mitk::Equal(CheckForNaNPoint(p2), true),".. Testing that CheckFormNaNPoint returns true for a point with double NaN x");

  MITK_TEST_CONDITION_REQUIRED(mitk::Equal(CheckForNaNPoint(p4), true),".. Testing that CheckFormNaNPoint returns true for a point with float NaN y");

  MITK_TEST_CONDITION_REQUIRED(mitk::Equal(CheckForNaNPoint(p5), true),".. Testing that CheckFormNaNPoint returns true for a point with double NaN z");

  MITK_TEST_CONDITION_REQUIRED(mitk::Equal(CheckForNaNPoint(p3), false),".. Testing that CheckFormNaNPoint returns false for a point with infinite x");

  MITK_TEST_CONDITION_REQUIRED(mitk::Equal(CheckForNaNPoint(p6), true),".. Testing that CheckFormNaNPoint returns true for a point with all NaNs");
...
void TestRMS()
{
  ...
  double rms = GetRMSErrorBetweenPoints(*fixedPoints, *movingPoints);
  ...
  rms = GetRMSErrorBetweenPoints(*fixedPoints, *movingPoints);
  ...
  rms = GetRMSErrorBetweenPoints(*fixedPoints, *movingPoints, transform.GetPointer());
  expected = 1;
...
/**
 * Basic test harness for niftkPointUtils
 */
int niftkPointUtilsTest(int argc, char * argv[])
{
  // always start with this!
  MITK_TEST_BEGIN("niftkPointUtilsTest");

  niftk::TestCalculateStepSize();
  niftk::TestAreDifferent();
  niftk::TestGetSquaredDistanceBetweenPoints();
  niftk::TestGetDifference();
  niftk::TestNormalise();
  niftk::TestComputeNormalFromPoints();
  niftk::TestFilterMatchingPoints();
  niftk::TestRMS();
  niftk::TestRemoveNaNPoints();
  niftk::TestCheckForNaNPoint();
  niftk::TestFindLargestDistanceBetweenTwoPoints();
  niftk::TestScalePointSets();

...
//-----------------------------------------------------------------------------
int mitkFindAndTriangulateCrossHairTest(int argc, char * argv[])
{
  mitk::FindAndTriangulateCrossHair::Pointer Finder = mitk::FindAndTriangulateCrossHair::New();
  Finder->SetVisualise(true);
  Finder->Initialise(argv[1], argv[2]);
  Finder->SetFlipMatrices(true);
  Finder->SetTrackerIndex(2);
  //check it initialised, check it gets the right matrix with the right time error
  MITK_TEST_CONDITION_REQUIRED (Finder->GetInitOK() , "Testing mitkFindAndTriangulateCrossHair Initialised OK"); 
 ...
 // Projector->SetWorldPoints(WorldGridPoints);
  Finder->Triangulate();
//  MITK_TEST_CONDITION_REQUIRED (Finder->GetProjectOK(), "Testing mitkProjectPointsOnStereoVideo projected OK"); 

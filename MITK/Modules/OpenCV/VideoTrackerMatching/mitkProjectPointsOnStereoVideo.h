 ...
 * the user can specify a set of points in model or world coordinates,
 * along with a model to world transform, that
...
private:
  ...
  mitk::PickedPointList::Pointer  m_WorldPoints;  //the world points to project, and their accompanying scalar values
  mitk::PickedPointList::Pointer  m_ModelPoints;  //the model points to transform to world space thenproject.
  ...
  cv::Mat* m_ModelToWorldTransform;
  ...
  mitk::ProjectedPointPairsWithTimingError              m_ScreenAxesPoints; // the projected axes points

    ...
    if (opt->helpmsg)
    {
     ...
     while (*cptr) 
     {
       ...
       else                   /* Print a word. */
       {
         ...
         if (newsent) 
         {        /* Extra space at start of sentence */
           fputc(' ', stdout);
           newsent = 0;
           length++;
         }
                                ...
                                struct CommandLineArgumentDescription *clArgList)
{
  ...
  for (arg = 1; arg < argc; arg++) 
  {
    ...
    && ! STREQUAL(argv[arg], "--")) 
    {
      ...
      if (!found) 
      {   
        /* Keys must be matched to programme
           command line specs */
...
void CommandLineParser::ParseStandardArguments(char *key, struct CommandLineArgumentDescription *clArgList)
{
  ...
  if (STREQUAL(key, "--version"))
  {
    std::cout << " version: " << NIFTK_VERSION_STRING << "\n";
    exit(0);
  }
}
...
void CommandLineParser::ReadParameter(char *argv[], int arg, int type, struct ml *mlptr)
{
  ...
  switch (type & OPT_TYPEMASK)
  {
    ...
    case OPT_LONG:
      ...
      if (*rest) 
        errmsg("Badly formatted integer number %s", argv[arg]);
      break;
    case OPT_FLOAT:
    case OPT_DOUBLE:
      chknotopt(argv,arg);
      ...
      if (*rest) 
        errmsg("Badly formatted real number %s",argv[arg]);
      break;
    case OPT_INTx2:
      chknotopt(argv, arg);
...
int *CommandLineParser::get_intx2(char *str)
{
  ...
  if (sscanf(str, "%d%*1[x,]%d", &intx2[0], &intx2[1]) != 2) 
    errmsg("Incorrectly formatted intx2 option %s, should be '<a>,<b>'.", str);
...
int *CommandLineParser::get_intx3(char *str)
{
  ...
  if (sscanf(str,"%d%*1[x,]%d%*1[x,]%d",
    ...
    errmsg("Incorrectly formatted intx3 option %s, should be '<a>,<b>,<c>'.", str);
...
float *CommandLineParser::get_floatx2(char *str)
{
  ...
  if (sscanf(str, "%f%*1[x,]%f", &floatx2[0], &floatx2[1]) != 2) 
    errmsg("Incorrectly formatted floatx2 option %s, should be '<a>,<b>'.", str);
...
double *CommandLineParser::get_doublex2(char *str)
{
  ...
  if (sscanf(str, "%lf%*1[x,]%lf", &doublex2[0], &doublex2[1]) != 2) 
    errmsg("Incorrectly formatted doublex2 option %s, should be '<a>,<b>'.", str);

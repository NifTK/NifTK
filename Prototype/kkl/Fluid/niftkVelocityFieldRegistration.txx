...
void StartUsage(char *name)
{
  ...
  std::cout << "  " << std::endl;
  std::cout << "  Implements LDDMM-like (velocity field) registration, initially based on Beg et al. 2005." << std::endl;
  std::cout << "  " << std::endl;
  ...
  std::cout << "    -to <filename> [fluid.dof]       Output tranformation" << std::endl << std::endl;    
}
...
void EndUsage()
{
  ...
  std::cout << "    -osi <filename>                    Output resampled fixed image" << std::endl << std::endl;  
  ...
  std::cout << "    -mc <float>       [1.0e-9]         Minimum change in cost function (NMI), " << std::endl;
  std::cout << "                                       below which we stop that resolution level." << std::endl;
  std::cout << "    -md <float>       [0.05]           Minimum change in deformation magnitude between iterations, " << std::endl;
  std::cout << "                                       below which we stop that resolution level." << std::endl;  
  ...
  std::cout << "    -ls <float>       [1.0]            Largest step size factor (voxel unit)" << std::endl;
  ...
  std::cout << "                                       3. BSpline" << std::endl;
  std::cout << "                                       4. Sinc" << std::endl;
  std::cout << "    -sim <int>        [9]              Choose similarity measure" << std::endl; 
  ...
  std::cout << "    -cs <int>         [1]              Check similarity during optimisation, 1 to check, 2 to skip. " << std::endl;
  ...
  std::cout << "    -force <string>   [force]          Registration force type" << std::endl; 
  std::cout << "                                       ssd - Christensen's SSD derived force (default)" << std::endl;
  ...
  std::cout << "    -abs_output <int> [0]              Output absoulte intensity value in resliced image" << std::endl;
  ...
  std::cout << "    -mdmi <double> <double> [-1 0.1]   Maximum number of iterations allowed for step size less than min deformation." << std::endl; 
  ...
  std::cout << "    -sym                               Symmetrically deform the fixed and floating image." << std::endl; 
  std::cout << "    -gs                                Geodesic shooting." << std::endl; 
  std::cout << "    -temp                              Temporary file directory." << std::endl; 
...
int velocityFieldMain(int argc, char** argv)
{
  ...
  std::string outputTransformedFixedImage;
  ...
  int checkSim = 1; 
  int dilations = 0;
  double minCostTol = 1.0e-9;
  double minDefChange = 0.05;
  double minJac = 0.5;
  double iReduce = 0.5;
  double rReduce = 1.0;
  double jReduce = 0.5;
  ...
  std::string registrationForceName = "ssd";  
  int sim = 9; 
  int isSmoothForce = 0;
  double mu = 0.01; 
  double lambda = 0.0;  
  ...
  for(int i=1; i < argc; i++){
    if(strcmp(argv[i], "-help")==0 || strcmp(argv[i], "-Help")==0 || strcmp(argv[i], "-HELP")==0 || strcmp(argv[i], "-h")==0 || strcmp(argv[i], "--h")==0){
      ...
      outputTransformedFixedImage=argv[++i];
      std::cout << "Set -osi=" << outputTransformedFixedImage << std::endl;
    }
    ...
    //}
      ...
      itk::VelocityFieldDeformableTransformFilename::SetTempPath(argv[++i]); 
      std::cout << "Set -temp=" << itk::VelocityFieldDeformableTransformFilename::GetTempPath() << std::endl;
    }
    else{
  ...
  if (fixedImage.length() <= 0 || movingImage.length() <= 0 || outputTransformation.length() <= 0)
  {
    StartUsage(argv[0]);
  ...
  typedef typename itk::VelocityFieldMultiResolutionMethod<InputImageType, double, Dimension, MultiResolutionPyramidImageFilterType>   MultiResImageRegistrationMethodType;
  typename MultiResImageRegistrationMethodType::Pointer multiResMethod = MultiResImageRegistrationMethodType::New();

  typedef typename itk::VelocityFieldDeformableTransform<InputImageType, double, Dimension, float > VelocityFieldDeformableTransformType;
  typename VelocityFieldDeformableTransformType::Pointer transform = VelocityFieldDeformableTransformType::New(); 
  ...
  typedef typename itk::VelocityFieldGradientDescentOptimizer<InputImageType, InputImageType, double, float> VelocityFieldGradientDescentOptimizerType;

  typename VelocityFieldGradientDescentOptimizerType::Pointer optimizer = VelocityFieldGradientDescentOptimizerType::New(); 
  ...
  optimizer->SetIsPropagateRegriddedMovingImage(false); 
  if (isGS)
  {
    optimizer->SetFixedImageInterpolator(factory->CreateInterpolator((typename itk::InterpolationTypeEnum)regInterp)); 
    optimizer->SetMovingImageInterpolator(factory->CreateInterpolator((typename itk::InterpolationTypeEnum)regInterp)); 
  }
  ...
  {
    typedef typename itk::ImageFileReader<typename VelocityFieldDeformableTransformType::DeformationFieldType> DeformationReaderType; 
  ...
  if (isSymmetric)
  {
    typename VelocityFieldDeformableTransformType::Pointer fixedTransform = VelocityFieldDeformableTransformType::New();
    fixedTransform->Initialize(inputFixedImage.GetPointer());
    ...
    metric->SetSymmetricMetric(FactoryType::MetricType::SYMMETRIC_METRIC_BOTH_FIXED_AND_MOVING_TRANSFORM); 
    ...
    optimizer->SetFixedImageTransform(fixedTransform); 
  }
  ...
  if (outputImage.length() > 0)
  {
    ...
    //if (isSymmetric)
    {
      // castFilter->SetInput(optimizer->GetRegriddedFixedImage()); 
      //outputImageWriter->SetFileName(outputTransformedFixedImage);
      //outputImageWriter->Update();      
    }
  }
  if (crop > 0)
  ...
  {
    typedef typename itk::ImageFileWriter< typename VelocityFieldDeformableTransformType::DeformationFieldType >  DeformationImageWriterType;
  ...
  std::cout << "Saving transform...done" << std::endl;

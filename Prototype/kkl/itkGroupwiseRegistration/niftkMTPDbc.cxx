...
int main(int argc, char** argv)
{
  ...
  try
  {
    ...
    double* mean = new double[numberOfImages];
    ...
    {
      requestedRegion.SetIndex(i, INT_MAX);
      ...
      for (int i = 0; i < numberOfImages; i++)
      {
        for (int j = i+2; j < numberOfImages; j++)
        {
          ...
          for (int m = i, n=i+1; m < j; m++,n++)
          {
            ...
            ImageIterator image1It(pairwiseBiasRatioImage[nonConsetiveTimePointIndex], 
                                   pairwiseBiasRatioImage[nonConsetiveTimePointIndex]->GetLargestPossibleRegion());
            ImageIterator image2It(medianImageFilter[consetiveTimePointIndex]->GetOutput(),
                                   medianImageFilter[consetiveTimePointIndex]->GetOutput()->GetLargestPossibleRegion());
    ...
    for (int i = 0; i < numberOfImages; i++)
    {
      ...
      for (int j = 0; j < numberOfImages; j++)
      {
        ...
        if (i != j)
        {
          ...
          ImageIterator image1It(biasRatioImage[i]->GetOutput(), biasRatioImage[i]->GetOutput()->GetLargestPossibleRegion());
          ImageIterator image2It(pairwiseBiasRatioImage[index], pairwiseBiasRatioImage[index]->GetLargestPossibleRegion());
          for (image1It.GoToBegin(), image2It.GoToBegin(); 
              !image1It.IsAtEnd(); 
              ++image1It, ++image2It)
          {

...
#
#  Last Changed      : $LastChangedDate: 2011-03-29 14:46:27 +0100 (Tue, 29 Mar 2011) $ 
#  Revision          : $Revision: 5719 $
...
Optional arguements:  
  ...
  -interpolation [int] : interpolation [4].
                         1. Nearest neighbour. 
                         2. Linear. 
                         3. Cubic.
                         4. Sinc. 
...
function iterate_through_input_file
{
  ...
  do
    ...
    local output_format="${study_number}_groupwise_%i"
    
    if [ ${do_or_check} == 1 ] 
    then 
      echo ${script_dir}/ComputeInitialAffineAtlas.sh \
           ${output_dir}/${output_format} \
           ${fixed_image} \
           ${fixed_image_mask} ${dilation} ${symmetric}  \
           ${dof} ${scaling_using_skull} ${similarity} ${ajc} ${region} \
           ${interpolation} \
           ${skip_registration} \
           ${moving_images_and_masks} >> ${command_filename}
    else

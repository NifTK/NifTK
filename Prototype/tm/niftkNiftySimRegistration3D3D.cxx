...
int DoMain(arguments args)
{
  ...
  { 
    if ( args.fileTargetInputImage.length() > 0 )
    {
      ...
      niftk::LogHelper::InfoMessage("Loading fixed image:" + args.fileTargetInputImage);
      fixedReader->Update();  
    ...
    if ( args.fileSourceInputImage.length() > 0 )
    {
      ...
      niftk::LogHelper::InfoMessage("Loading moving image:" + args.fileSourceInputImage);
      movingReader->Update();  
      niftk::LogHelper::InfoMessage("Done");         
  ...
  if ( args.fileTransformedOutputImage.length() ) {
    ...
    try
      { 
	niftk::LogHelper::InfoMessage(std::string("Writing transformed image to file: ") 
  ...
  if ( args.fileOutputMask.length() > 0 ) {
    niftk::LogHelper::InfoMessage(std::string("Writing the mask to file: ") 
				  + args.fileOutputMask );
  ...
  if ( args.fileOutputNodalDisplacements.length() > 0 ) {
    niftk::LogHelper::InfoMessage(std::string("Writing the nodal displacements to file: ") 
  ...
  if ( args.fileOutputModel.length() > 0 ) {
    niftk::LogHelper::InfoMessage(std::string("Writing the model to file: ") 
  ...
  switch ( dims )
    {
      ...
      break;
      default:
	niftk::LogHelper::ErrorMessage( "Unsupported image dimension");
        exit( EXIT_FAILURE );
    }
  return result;
}

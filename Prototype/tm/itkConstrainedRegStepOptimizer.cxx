/*=============================================================================
 ...
 ============================================================================*/
#ifndef _itkConstrainedRegStepOptimizer_cxx
#define _itkConstrainedRegStepOptimizer_cxx
...
log4cplus::Logger 
ConstrainedRegStepOptimizer::s_Logger(
log4cplus::Logger::getInstance("ConstrainedRegStepOptimizer"));
...
::ConstrainedRegStepOptimizer()
{
  ...
  LOG4CPLUS_DEBUG(s_Logger, "Constructed:ConstrainedRegStepOptimizer()");
}
  ...
  while( !m_Stop ) 
  {
    ...
    try
    {
      ...
      for (unsigned int parameterIndex = 0; parameterIndex < currentParameters.Size(); parameterIndex++)
      {
        ...
        if ( parameterIndex > 3 ) // check contraint boundaries
	  {
            switch (parameterIndex) {
	      case 4: // displacement
                if ( currentParameters[parameterIndex] > dispMax )
                  currentParameters[parameterIndex] = dispMax;
                if ( currentParameters[parameterIndex] < dispMin )
                  currentParameters[parameterIndex] = dispMin;
                break;
	      case 5: // anisotropy
                if ( currentParameters[parameterIndex] > anisoMax )
                  currentParameters[parameterIndex] = anisoMax;
                if ( currentParameters[parameterIndex] < anisoMin )
                  currentParameters[parameterIndex] = anisoMin;
                break;
	      case 6: // Poisson's ratio
                if ( currentParameters[parameterIndex] > poiMax )
                  currentParameters[parameterIndex] = poiMax;
                if ( currentParameters[parameterIndex] < poiMin )
                  currentParameters[parameterIndex] = poiMin;
                break;
              default:
		std::cout << "Error! Parameter index: " << parameterIndex << " invalid!" << std::endl;
            }
	  } 
        this->m_Value = 0; 
        try

...
#ifndef __itkInvRayCastInterpolateCorridorOutput_txx
#define __itkInvRayCastInterpolateCorridorOutput_txx
...
public:
  ...
  /// Set File names for images used for debugging
  void SetCorFileNames( std::string beforeName, std::string afterName )
  { 
    m_corBeforeFileName = beforeName;
    m_neighCorAfterFileName = afterName;
  } 
...
protected:
  ...
  std::string m_neighCorAfterFileName;
  std::string m_corBeforeFileName;
...
::SaveDebugVolume(void)
{
   ...
   writer1->SetFileName( m_corBeforeFileName.c_str() );
   writer2->SetFileName( m_neighCorAfterFileName.c_str() );
   ...
   try 
   { 
     std::cout << "Writing corridor images..." << std::endl;
     writer1->Update();
     writer2->Update();
   } 
...
::IncrementVoxelPointers(void)
{
  ...
  if (useDebugVolume)
  {
    this->m_DebugImageBefore->SetPixel( pixelIndexBefore, 1 );//25000 );//After, 1 );//70 );   
    this->m_DebugImageBefore->Modified();

    //this->m_DebugImageAfter->SetPixel( pixelIndexAfter, 1 );//25000 );// dz );//70 );
...
::GetCurrentIntensity(void) const
{
  ...
  for( unsigned int counter = 0; counter < 8; counter++ )//m_Neighbors is 8 for this case
  {
    ...
    // Used to display where the values come from
    if (useDebugVolume)
    {
     //std::cout<<"Setting the neighbour:"<<neighIndex[0]<<" "<< neighIndex[1]<<" "<<neighIndex[2]<<std::endl;  
      this->m_DebugImageAfter->SetPixel( neighIndex, 1 );//0+counter );//(10+counter) );
      this->m_DebugImageAfter->Modified();
    }
...
template<class TInputImage, class TCoordRep>
InvRayCastInterpolateCorridorOutput< TInputImage, TCoordRep >
::InvRayCastInterpolateCorridorOutput()
{
  m_Threshold = 0.;
...
template<class TInputImage, class TCoordRep>
typename InvRayCastInterpolateCorridorOutput< TInputImage, TCoordRep >
::OutputType
InvRayCastInterpolateCorridorOutput< TInputImage, TCoordRep >
...
/* -----------------------------------------------------------------------
   Evaluate at pre-specified image index position
   ----------------------------------------------------------------------- */
...
::OutputType
InvRayCastInterpolateCorridorOutput< TInputImage, TCoordRep >
::EvaluateForOnePosition( const PointType& point ) const
{
  double integral = 0;
  
  useDebugVolume = 1;

  OutputPointType transformedFocalPoint = m_FocalPoint;
  DirectionType direction = transformedFocalPoint - point;
  ...
  ray.ZeroState();
  ray.Initialise();
  ray.SetCorFileNames( m_beforeFileName, m_neighAfterFileName );

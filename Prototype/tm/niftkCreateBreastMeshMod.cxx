...
struct niftk::CommandLineArgumentDescription clArgList[] = {
   ...
   {OPT_STRING, "imageBeforeVtk", "fileBeforeVtkOut", "Filename of the image before converting to vtk."},////////////////
...
int main (int argc, char *argv[])
{
  ...
  std::string fileOutputBeforeVtk; /////////////////////////
  ...
  if (CommandLineOptions.GetArgument(O_NITERATIONS, niterations) && (niterations > 0)
    ...
    niftk::LogHelper::WarningMessage(std::string("No. of smoothing iterations specified (")
                                  + niftk::ConvertToString(niterations) 
                                  + ") post-decimation smoothing assumed.");
    postSmooth = true;
  }

  if ((niterations < 0) || (niterations > 200)) {
    niftk::LogHelper::ErrorMessage(std::string("No. of smoothing iterations (")
                                     + niftk::ConvertToString(niterations)
  ...
  if (CommandLineOptions.GetArgument(O_BANDWIDTH, bandwidth) 
      && (! (preSmooth || postSmooth))) {
    niftk::LogHelper::WarningMessage("WARNING: Smoothing bandwith specified post-decimation smoothing assumed.");
    postSmooth = 1;
  }

  if ((bandwidth < 0.) || (bandwidth > 2)) {
    niftk::LogHelper::ErrorMessage(std::string("Smoothing bandwith (")
                                               + niftk::ConvertToString(bandwidth) 
  ...
  if (CommandLineOptions.GetArgument(O_USE_VTK_DECIMATE_PRO, flgDecimatePro) && verbose)
    niftk::LogHelper::InfoMessage("Using 'vtkDecimatePro' decimation [default: vtkQuadricDecimation]");

  if (CommandLineOptions.GetArgument(O_DECIMATION, decimation) && decimation && verbose)
    niftk::LogHelper::InfoMessage(std::string("Decimation factor: ")
                                  + niftk::ConvertToString(decimation) + " applied");

  if (decimation && ((decimation < 0.) || (decimation > 1.))) {
    niftk::LogHelper::ErrorMessage(std::string("Decimation value (")
                                   + niftk::ConvertToString(decimation)
                                   + ") is outside allowed range (0-1)");
    return EXIT_FAILURE;
  }

  if (CommandLineOptions.GetArgument(O_FEATUREANGLE, featureAngle) && verbose)
    niftk::LogHelper::InfoMessage(std::string("Decimation feature angle: ")
                                  + niftk::ConvertToString(featureAngle));

  if (featureAngle && ((featureAngle < 0.) || (featureAngle > 90.))) {
    niftk::LogHelper::ErrorMessage(std::string("Decimation feature angle value (") 
                                   + niftk::ConvertToString(featureAngle)
  ...
  CommandLineOptions.GetArgument(O_BEFOREVTK_OUTPUT_FILE, fileOutputBeforeVtk);////////////
  ...
  if (CommandLineOptions.GetArgument(O_TEXT_OUTPUT, flgTextOutput) && verbose)
      niftk::LogHelper::InfoMessage("Output vtk surface will be in TEXT format");
  else
      niftk::LogHelper::InfoMessage("Output vtk surface will be in BINARY format");
  ...
  niftk::LogHelper::InfoMessage(std::string("Input image resolution: ") 
  ...
  niftk::LogHelper::InfoMessage(std::string("Input image dimensions: ") 
  ...
  niftk::LogHelper::InfoMessage(std::string("Input image intensity range: ") 
  ...
  try
  { 
    niftk::LogHelper::InfoMessage(std::string("Applying pre-region-growing smoothing, no. iters: ")
  ...
  try
  { 
    niftk::LogHelper::InfoMessage("Region-growing the image background between: "
                                  + niftk::ConvertToString(imMinimum) + " and "
                                  + niftk::ConvertToString(isoSurface) + "...");
  ...
  if ( fileOutputMask.length() > 0 ) {
      ...
      try {
        niftk::LogHelper::InfoMessage(std::string("Writing the segmentation to file: ") + fileOutputMask.c_str());
        writerMask->Update();
      }
  ...
  if (( planePosition || planeNormal ) && ( fileOutputPlane.length() > 0 )) {
    ...
    try
    { 
      niftk::LogHelper::InfoMessage("Thresholding mask voxels on one side of a plane...");
    ...
    try {
      niftk::LogHelper::InfoMessage(std::string("Writing mask thresholded by plane to file: ") + fileOutputPlane.c_str());
      writer->Update();
    }
    catch( itk::ExceptionObject & excep ) {
      std::cerr << "Exception caught !" << std::endl;
  ...
  try
  { 
    niftk::LogHelper::InfoMessage("Blurring the region-grown segmentation...");
  ...
  try
  { 
    niftk::LogHelper::InfoMessage(std::string("Resampling image to dimensions: ")
                                  ...
                                  + niftk::ConvertToString(spacing[2]) + "mm...");
  ...
  if ( distTransThresh ) {
    ...
    try
    { 
      niftk::LogHelper::InfoMessage("Computing distance transform...");
    ...
    if ( fileOutputDistanceTransform.length() > 0 ) {
      ...
      try {
        niftk::LogHelper::InfoMessage(std::string("Writing distance transform to file: ") 
                                      + fileOutputDistanceTransform.c_str());
        writer->Update();
      }
      catch( itk::ExceptionObject & excep ) {
        std::cerr << "Exception caught !" << std::endl;
  ...
  try
  { 
    niftk::LogHelper::InfoMessage("Sealing the image boundary...");
    setBoundary->Update();
  }
  catch (itk::ExceptionObject &ex)
  { 
  ...
  if ( planePosition || planeNormal ) {
    ...
    try
    { 
      niftk::LogHelper::InfoMessage("Thresholding voxels on one side of a plane...");
      thresholdWithRespectToPlane->Update();
    }
    catch (itk::ExceptionObject &ex)
    { 
  ...
  ///////////////////////////////////////////////////
  // Output the itk image before converting to vtk
  ///////////////////////////////////////////////////
  if ( fileOutputBeforeVtk.length() > 0 ) {
      
    typedef  itk::ImageFileWriter<  InputImageType  > WriterType;
    WriterType::Pointer writer = WriterType::New();
      
    writer->SetFileName( fileOutputBeforeVtk );
    writer->SetInput( pipeITKImageDataConnector );
      
    try {
      niftk::LogHelper::InfoMessage(std::string("Writing image before converted to vtk to file: ") + fileOutputBeforeVtk.c_str());
      writer->Update();
    }
    catch( itk::ExceptionObject & excep ) {
      std::cerr << "Exception caught !" << std::endl;
  ...
  try
  { 
    niftk::LogHelper::InfoMessage("Converting the image to VTK format...");
  ...
  if ( fileOutputPolydata.length() > 0 ) {
    ...
    if (verbose) 
      niftk::LogHelper::InfoMessage(std::string("Polydata written to VTK file: ") + fileOutputPolydata.c_str());
  }
  ...
  if ( fileOutputSTL.length() > 0 ) {
    ...
    if (verbose) 
      niftk::LogHelper::InfoMessage(std::string("Polydata written to STL file: ") + fileOutputPolydata.c_str());
  }
  ...
  if ( fileOutputSMESH.length() > 0 ) {
    ...
    if (verbose) 
      niftk::LogHelper::InfoMessage(std::string("Surface data written to SMESH file: ") + fileOutputSMESH.c_str());
  }
...
void WritePolydataAsTetgenSmeshFile( const char *smesh_file, vtkPolyData *surface )
{
  ...
  if (fp->fail()) {
    niftk::LogHelper::ErrorMessage( std::string("Could not open smesh file: ") + smesh_file);
    return;
  }
  ...
  *fp << "# Node index, node coordinates" << endl;
  ...
  default:
  {
    niftk::LogHelper::ErrorMessage( "Could not write smesh data type");
    return;
  }
  ...
  *fp << "# Number of nodes defining the facet, node indices" << endl;
  ...
  if (ncells < 1) {
    niftk::LogHelper::ErrorMessage( "No cells in surface mesh, SMESH file save aborted." );
    return;
  }

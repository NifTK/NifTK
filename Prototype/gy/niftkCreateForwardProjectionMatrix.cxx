...
#include "itkCreateForwardProjectionMatrix.h"
...
int main(int argc, char** argv)
{
  ...
  typedef itk::CreateForwardProjectionMatrix< IntensityType > ForwardProjectorType;
  ...
  for(int i=1; i < argc; i++){
    if(strcmp(argv[i], "-help")==0 || strcmp(argv[i], "-Help")==0 || strcmp(argv[i], "-HELP")==0 
      ...
      std::cout << std::string("Set -p=") << filePerspectiveTransform;
    }
    else if(strcmp(argv[i], "-f") == 0) {
      focalLength = atof(argv[++i]);
      std::cout << std::string("Set -f=") << niftk::ConvertToString(focalLength);
    }
    else if(strcmp(argv[i], "-u0") == 0) {
      u0 = atof(argv[++i]);
      std::cout << std::string("Set -u0=") << niftk::ConvertToString(u0);
    }
    else if(strcmp(argv[i], "-v0") == 0) {
      v0 = atof(argv[++i]);
      std::cout << std::string("Set -v0=") << niftk::ConvertToString(v0);
    }
    else if(strcmp(argv[i], "-t") == 0) {
      threshold = atof(argv[++i]);
      std::cout << std::string("Set -t=") << niftk::ConvertToString(threshold);
    }
  ...
  try { 
    std::cout << std::string("Writing output to file: ") << fileOutputImage2D;
    writer->Update();
    std::cout << std::string("Done");
  } 
  catch( itk::ExceptionObject & err ) { 

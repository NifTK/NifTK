...
void Usage(char *exec)
{
  ...
  std::cout << "  " << std::endl
    << "    Program of performing a simultaneous unconstrained matrix-based reconstruction and registration" << std::endl
    << "    from two set of projection images and an initial estimate (or zero)." << std::endl << std::endl
...
int main(int argc, char** argv)
{
  ...
  typedef itk::SimultaneousUnconstrainedMatrixReconRegnMethod <IntensityType> 									SimultaneousUnconstrainedReconRegnMethodType;
  typedef SimultaneousUnconstrainedReconRegnMethodType::SimultaneousUnconstrainedReconRegnType  SimultaneousUnconstrainedReconRegnType;
  ...
  std::string fileOutputSimultaneousUnconstrainedReconRegn3D;
  typedef SimultaneousUnconstrainedReconRegnMethodType::SimultaneousUnconstrainedReconRegnType	OutputSimultaneousUnconstrainedReconRegn3D;	
  typedef OutputSimultaneousUnconstrainedReconRegn3D::Pointer      															OutputSimultaneousUnconstrainedReconRegnPointer;
  typedef OutputSimultaneousUnconstrainedReconRegn3D::RegionType   															OutputSimultaneousUnconstrainedReconRegnRegionType;
  typedef OutputSimultaneousUnconstrainedReconRegn3D::PixelType    															OutputSimultaneousUnconstrainedReconRegnPixelType;
  typedef OutputSimultaneousUnconstrainedReconRegn3D::SizeType     															OutputSimultaneousUnconstrainedReconRegnSizeType;
  typedef OutputSimultaneousUnconstrainedReconRegn3D::SpacingType  															OutputSimultaneousUnconstrainedReconRegnSpacingType;
  typedef OutputSimultaneousUnconstrainedReconRegn3D::PointType   					 										OutputSimultaneousUnconstrainedReconRegnPointType;
  typedef OutputSimultaneousUnconstrainedReconRegn3D::IndexType    															OutputSimultaneousUnconstrainedReconRegnIndexType;
  ...
  // (Here the affine transform is applied to simulate a pair of DBT acquisitions)
...
*/
  ...
  // Initial guess of the affine parameters 
	// (Try to pass the initial guess to the 'Metric' but it seems these 
  // parameters can't be updated properly)
  EulerAffineTransformType::ParametersType parametersInitialGuess;

  // parameters 0-2: translation; 3-5: rotation; 6-8 scaling; 9-11: skew
  parametersInitialGuess.SetSize(12); 					
  parametersInitialGuess.Fill(1.1);
  ...
  for(int i=1; i < argc; i++){
    if(strcmp(argv[i], "-help")==0 || strcmp(argv[i], "-Help")==0 || strcmp(argv[i], "-HELP")==0 
      ...
      fileOutputSimultaneousUnconstrainedReconRegn3D = argv[++i];
      std::cout << "Set -o=" << fileOutputSimultaneousUnconstrainedReconRegn3D;
    }
  ...
  if ( nVoxels3D[0] == 0 || nVoxels3D[1] == 0 || nVoxels3D[2] == 0 || 
      fileInputImage3D.length() == 0 || fileOutputSimultaneousUnconstrainedReconRegn3D.length() == 0 || 
  ...
  simultaneousUnconMetric->SetInputVolume( inImageEstimation );
	...
	simultaneousUnconMetric->SetInputVolumeVector( initialEstimateImageVector );

	// Set the transformation parameters as a vector form
  simultaneousUnconMetric->SetParameterVector( initialParametersVector );
  ...
  simultaneousUnconMetric->SetInputVolumeSize( nVoxels3D );
  simultaneousUnconMetric->SetInputVolumeSpacing( spacing3D );
  simultaneousUnconMetric->SetInputVolumeOrigin( origin3D );


  // Create the reconstructor/Registor
  // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

  SimultaneousUnconstrainedReconRegnMethodType::Pointer imReconRegnator = SimultaneousUnconstrainedReconRegnMethodType::New();
    ...
    case OPTIMIZER_CONJUGATE_GRADIENT_MAXITER:
      {
        ...
        OptimizerType::Pointer optimizer = OptimizerType::New();
				...
				itk::ConjugateGradientMaxIterOptimizer::ScalesType scales( parameters.Size() + totalSize3D ); 
...
*/
        ...
        imReconRegnator->SetOptimizer( optimizer );
        break;
      }
    ...
    case OPTIMIZER_LIMITED_MEMORY_BFGS:
      {
        ...
        OptimizerType::Pointer optimizer = OptimizerType::New();
				...
				itk::LBFGSOptimizer::ScalesType scales( parameters.Size() + totalSize3D ); 
    ...
    case OPTIMIZER_REGULAR_STEP_GRADIENT_DESCENT:
      {
        ...
        OptimizerType::Pointer optimizer = OptimizerType::New();
				...
				itk::RegularStepGradientDescentOptimizer::ScalesType scales( parameters.Size() + totalSize3D );
...
*/
  ...
  imReconRegnator->SetSimultaneousUnconReconRegnVolumeSize( nVoxels3D );
  imReconRegnator->SetSimultaneousUnconReconRegnVolumeSpacing( spacing3D );
  imReconRegnator->SetSimultaneousUnconReconRegnVolumeOrigin( origin3D );

  // Set the geometry
  // ~~~~~~~~~~~~~~~~

  imReconRegnator->SetProjectionGeometry( geometry );

  // Set the metric
  // ~~~~~~~~~~~~~~

  imReconRegnator->SetMetric( simultaneousUnconMetric );
  ...
  try
  {
    std::cout << "Starting simultaneous unconstrained reconstruction and registration...";

    if (flgDebug)
      cout << "SimultaneousUnconstrainedMatrixReconRegnMethod: " << imReconRegnator << endl;

    imReconRegnator->Update();
    std::cout << "Simultaneous unconstrained reconstruction and registration complete";
  }
	catch( itk::ExceptionObject & err )
	{
   	std::cerr << "ERROR: Failed to calculate the simultaneous unconstrained reconstruction and registration; " << err << endl;
   	return EXIT_FAILURE;
 	}
  ...
  // Write the output reconstruction to a file 
	// (Currently this outputs nothing. 
	// By saving the results as .txt files, we can view the results using Matlab)
  // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

  typedef itk::ImageFileWriter< OutputSimultaneousUnconstrainedReconRegn3D > OutputSimultaneousUnconstrainedReconRegn3DWriterType;

  OutputSimultaneousUnconstrainedReconRegn3DWriterType::Pointer writer = OutputSimultaneousUnconstrainedReconRegn3DWriterType::New();

  writer->SetFileName( fileOutputSimultaneousUnconstrainedReconRegn3D );
  writer->SetInput( imReconRegnator->GetOutput() );

  try {
    std::cout << "Writing output to file: " << fileOutputSimultaneousUnconstrainedReconRegn3D;
    writer->Update();
  }
  catch( itk::ExceptionObject & err ) {
    std::cerr << "ERROR: Failed to write output to file: " << fileOutputSimultaneousUnconstrainedReconRegn3D << "; " << err << endl;
    return EXIT_FAILURE;
  }

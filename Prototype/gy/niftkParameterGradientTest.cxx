...
void Usage(char *exec)
{
  ...
  std::cout << "  " << std::endl
    ...
    << "    -iin  <filename>                  Input a arbitrary 3D image volume to perform the inner product " << std::endl
		...
		<< "    -opl  <filename>                  Output the moving plus image (Affine transformed with add some change 'R_plus(x)')" << std::endl
		<< "    -oq   <filename>                  Output the f2(x, p, h)" << std::endl
    ...
    << "    -iga  <doublex12>                 Input the 12 transformation parameters as the initial guess directly" << std::endl
		...
		<< "    -idh  <doublex12>                 Input the 12 difference values in order to change the original parameters" << std::endl << std::endl
...
int main(int argc, char** argv)
{
  ...
  std::string fileMovingPlusImage3D;
  std::string fileQImage3D;
  ...
  EulerAffineTransformType::ParametersType parameters(12);
	...
	parameters[4] = 10.;  // Roation along the 'x' axis
  ...
  parameters[8] = 1.;		// Scale factor along the 'z' axis
	...
	EulerAffineTransformType::ParametersType parametersDiffh(12);
#if 0
	parametersDiffh[0] 		= 1.0;
	parametersDiffh[1] 		= 1.0;
	parametersDiffh[2] 		= 2.0;
	parametersDiffh[3] 		= 2.0;
	parametersDiffh[4] 		= 1.0;
	parametersDiffh[5] 		= -5.0;
	parametersDiffh[6] 		= 0.01;
	parametersDiffh[7] 		= 0.02;
	parametersDiffh[8] 		= 0.03;
	parametersDiffh[9] 		= 0.01;
	parametersDiffh[10] 	= 0.02;
	parametersDiffh[11] 	= 0.03;
#endif
	parametersDiffh[0] 		= 0.01;
	parametersDiffh[1] 		= 0.01;
	parametersDiffh[2] 		= 0.02;
	parametersDiffh[3] 		= 0.01;
	parametersDiffh[4] 		= 0.01;
	parametersDiffh[5] 		= -0.05;
	parametersDiffh[6] 		= 0.0001;
	parametersDiffh[7] 		= 0.0002;
	parametersDiffh[8] 		= 0.0003;
	parametersDiffh[9] 		= 0.0001;
	parametersDiffh[10] 	= 0.0002;
	parametersDiffh[11] 	= 0.0003;

	// EulerAffineTransformType::ParametersType parametersDiffh(12);
	// parametersDiffh.Fill(0.0000001);
  ...
  for(int i=1; i < argc; i++){
    if(strcmp(argv[i], "-help")==0 || strcmp(argv[i], "-Help")==0 || strcmp(argv[i], "-HELP")==0 
      ...
      fileMovingPlusImage3D = argv[++i];
      std::cout << "Set -opl=" + fileMovingPlusImage3D;
    }

    else if(strcmp(argv[i], "-oq") == 0) {
      fileQImage3D = argv[++i];
      std::cout << "Set -oq=" << fileQImage3D;
    }
      ...
      parametersInitialGuess[0] 		= atof(argv[++i]);
      parametersInitialGuess[1] 		= atof(argv[++i]);
      parametersInitialGuess[2] 		= atof(argv[++i]);
      parametersInitialGuess[3] 		= atof(argv[++i]);
      parametersInitialGuess[4] 		= atof(argv[++i]);
      parametersInitialGuess[5] 		= atof(argv[++i]);
      parametersInitialGuess[6] 		= atof(argv[++i]);
      parametersInitialGuess[7] 		= atof(argv[++i]);
      parametersInitialGuess[8] 		= atof(argv[++i]);
      parametersInitialGuess[9] 		= atof(argv[++i]);
      parametersInitialGuess[10] 		= atof(argv[++i]);
      parametersInitialGuess[11] 		= atof(argv[++i]);
      std::cout << "Set -iga="
      ...
      std::cout << "Set -iss="
				...
				<< niftk::ConvertToString(scaleParameter[11]);
    }

		// Set the difference vector h (optional)

    else if(strcmp(argv[i], "-idh") == 0) {
			parametersDiffh[0] 	= atof(argv[++i]);
			parametersDiffh[1] 	= atof(argv[++i]);
			parametersDiffh[2] 	= atof(argv[++i]);
			parametersDiffh[3] 	= atof(argv[++i]);
			parametersDiffh[4] 	= atof(argv[++i]);
			parametersDiffh[5] 	= atof(argv[++i]);
			parametersDiffh[6] 	= atof(argv[++i]);
			parametersDiffh[7] 	= atof(argv[++i]);
			parametersDiffh[8] 	= atof(argv[++i]);
			parametersDiffh[9] 	= atof(argv[++i]);
			parametersDiffh[10] = atof(argv[++i]);
			parametersDiffh[11] = atof(argv[++i]);
      std::cout << "Set -idh="
				<< niftk::ConvertToString(parametersDiffh[0]) << " "
				<< niftk::ConvertToString(parametersDiffh[1]) << " "
				<< niftk::ConvertToString(parametersDiffh[2]) << " "
				<< niftk::ConvertToString(parametersDiffh[3]) << " "
				<< niftk::ConvertToString(parametersDiffh[4]) << " "
				<< niftk::ConvertToString(parametersDiffh[5]) << " "
				<< niftk::ConvertToString(parametersDiffh[6]) << " "
				<< niftk::ConvertToString(parametersDiffh[7]) << " "
				<< niftk::ConvertToString(parametersDiffh[8]) << " "
				<< niftk::ConvertToString(parametersDiffh[9]) << " "
				<< niftk::ConvertToString(parametersDiffh[10]) << " "
				<< niftk::ConvertToString(parametersDiffh[11]);
    }
  ...
  if ( 		 (nVoxels3D[0] 																	== 0) 
				...
				|| (fileMovingPlusImage3D.length() 								== 0)
				|| (fileQImage3D.length() 							          == 0)
  ...
  matrixProjector->CalculteMatrixVectorMultiplication(forwardProjectionMatrix, affineTransformedVector, forwardProjectedVectorTwo);
	...
	EulerAffineTransformType::ParametersType parametersPlus(12);	
  parametersPlus.Fill(0.);

	// Test f1: f1(x, p+h) - f1(x, p-h):: Here we output affineTransformedGradientVector
	// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

	double epsilonVal = 0.00001;

	for (unsigned int iPara = 0; iPara < parameters.Size(); iPara++)
	{
		parametersPlus[iPara] = parameters[iPara] + parametersDiffh[iPara];
	}
  ...
  affineTransformer->GetAffineTransformationSparseMatrix(affineMatrixPlus, nVoxels3D, parametersPlus);
	affineMatrixPlus.subtract(affineMatrix, affineMatrixGradient);

#if 1
  // Calculate the affine transform for the original image
  // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  VectorType affineTransformedOriginalInputImage(totalSize3D);
  affineTransformedOriginalInputImage.fill(0.);
  affineTransformer->CalculteMatrixVectorMultiplication(affineMatrix, inputImageVector, affineTransformedOriginalInputImage);

  // Calculate the 'plus' affine transform for the original image
  // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  VectorType plusAffineTransformedOriginalInputImage(totalSize3D);
  plusAffineTransformedOriginalInputImage.fill(0.);
  affineTransformer->CalculteMatrixVectorMultiplication(affineMatrixPlus, inputImageVector, plusAffineTransformedOriginalInputImage);

	// Get the gradient
  // ~~~~~~~~~~~~~~~~
  VectorType affineTransformedGradientVector(totalSize3D);
  affineTransformedGradientVector.fill(0.);
	affineTransformedGradientVector = plusAffineTransformedOriginalInputImage - affineTransformedOriginalInputImage;
#endif
  ...
  affineTransformer->CalculteMatrixVectorMultiplication(affineMatrixGradient, inputImageVector, affineTransformedGradientVector);
	// affineTransformedGradientVector /= epsilonVal;
#endif
  ...
  // Calculate the matrix/vector multiplication in order to get the plus moving image
  // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
  VectorType affineTransformedMovingPlusVector(totalSize3D);
  VectorType affineTransformedMovingMinusVector(totalSize3D);
  affineTransformedMovingPlusVector.fill(0.);
	affineTransformer->CalculteMatrixVectorMultiplication(affineMatrixPlus, inputImageVector, affineTransformedMovingPlusVector);

	// Test f2: f2(x, p, h):: Here we need a matrix Qk
	// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

	VectorType gradientImage(totalSize3D);
  gradientImage.fill(0.);

	VectorType gradientImagePH(totalSize3D);
  gradientImagePH.fill(0.);

	VectorType gradientImageTemp(totalSize3D);
  gradientImageTemp.fill(0.);

	VectorType vectorAdjointOutput(12);
  vectorAdjointOutput.fill(0.);

	VectorType vectorWithArbitraryImage(totalSize3D);
  vectorWithArbitraryImage.fill(0.);

  static SparseMatrixType affineMatrixQ(totalSize3D, totalSize3D);
  static SparseMatrixType affineMatrixQPlus(totalSize3D, totalSize3D);
  static SparseMatrixType affineMatrixQT(totalSize3D, totalSize3D);
  ...
  EulerAffineTransformType::ParametersType parametersTempPlus(12);
	...
	for (unsigned int iPara = 0; iPara < parameters.Size(); iPara++)
	{
		parametersTempPlus = parameters;
		parametersTempPlus[iPara] += epsilonVal;

		affineTransformer->GetAffineTransformationSparseMatrix(affineMatrixQPlus, nVoxels3D, parametersTempPlus);
		affineMatrixQPlus.subtract(affineMatrix, affineMatrixQ);

		affineTransformer->GetAffineTransformationSparseMatrixT(affineMatrixQ, affineMatrixQT, nVoxels3D);

		affineTransformer->CalculteMatrixVectorMultiplication(affineMatrixQ, inputImageVector, gradientImageTemp);
		gradientImagePH += parametersDiffh[iPara]*gradientImageTemp;
		gradientImage += gradientImageTemp;

		affineTransformer->CalculteMatrixVectorMultiplication(affineMatrixQT, inputImageArbitraryVector, vectorWithArbitraryImage);
  	vectorAdjointOutput[iPara] = dot_product( inputImageVector, vectorWithArbitraryImage );
	}

 	// Perform the inner product of the arbitrary volume with the gradient of affine transformation (r^T) .* (R'(x, p, h))
  // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

	double innerProductFowardOperator = 0.0;
  innerProductFowardOperator = dot_product( inputImageArbitraryVector, gradientImagePH );

	// Get another inner product (h^T) .* (R'^T(x)r) 
	// ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

	double innerProductAdjointOperator = 0.0;
  innerProductAdjointOperator = dot_product( parametersDiffh, vectorAdjointOutput );

	gradientImage = gradientImage*12;

  // Check the results
	// ~~~~~~~~~~~~~~~~~

	std::cerr << "Inner product (r^T) .* (R'(x, p, h)) is: " << innerProductFowardOperator 
						<< " should be equivalent to (h^T) .* (R'^T(x)r): " << innerProductAdjointOperator << std::endl;

  std::ofstream innerProductResultsFile("innerProductResultsFile.txt", std::ios::out | std::ios::app | std::ios::binary);
  innerProductResultsFile << "Inner product (r^T) .* (R'(x, p, h)) is: " << innerProductFowardOperator 
						<< " should be equivalent to (h^T) .* (R'^T(x)r): " << innerProductAdjointOperator << std::endl;

  // Check the vector h
	// ~~~~~~~~~~~~~~~~~~

  std::ofstream diffVectorHFile("diffVectorHFile.txt", std::ios::out | std::ios::app | std::ios::binary);
  diffVectorHFile << "Vector h is: [" << parametersDiffh[0] << " " << parametersDiffh[1] 	<< " " << parametersDiffh[2] 	<< " "
																						 	<< parametersDiffh[3] << " " << parametersDiffh[4] 	<< " " << parametersDiffh[5] 	<< " "
																						 	<< parametersDiffh[6] << " " << parametersDiffh[7] 	<< " " << parametersDiffh[8] 	<< " "
																						 	<< parametersDiffh[9] << " " << parametersDiffh[10] << " " << parametersDiffh[11] << "]" 
																						 	<< std::endl;
  ...
  // Moving plus image
  if (fileMovingPlusImage3D.length() > 0) {

		// Construct the output moving image and copy in the voxel values from the image vector
  	OutputImagePointer outputMovingPlusImage = OutputImageType::New();
  	...
  	outputMovingPlusImage->SetRegions( 	outputRegion );
  	outputMovingPlusImage->SetOrigin( 	origin3D );
  	outputMovingPlusImage->SetSpacing( 	spacing3D );

  	outputMovingPlusImage->Allocate();

		unsigned int iVoxel = 0; 
  	OutputImageConstIteratorType outputMovingPlusImageIter( outputMovingPlusImage, outputMovingPlusImage->GetLargestPossibleRegion() );
		for ( outputMovingPlusImageIter.GoToBegin(); !outputMovingPlusImageIter.IsAtEnd(); ++outputMovingPlusImageIter )
		{			
			outputMovingPlusImage->SetPixel( outputMovingPlusImageIter.GetIndex(), ((float) affineTransformedMovingPlusVector.get(iVoxel)) );
			iVoxel++;
		}

  	// Then write the image

  	OutputImageWriterType::Pointer outputMovingPlusImageWriter = OutputImageWriterType::New();

  	outputMovingPlusImageWriter->SetFileName( fileMovingPlusImage3D );
  	outputMovingPlusImageWriter->SetInput( 		outputMovingPlusImage );

  	try {
    	std::cout << "Writing moving image output to file: " << fileMovingPlusImage3D;
    	outputMovingPlusImageWriter->Update();
  	}
  	catch( itk::ExceptionObject & err ) {
    	std::cerr << "ERROR: Failed to write moving image output to file: " << fileMovingPlusImage3D << "; " << err << endl;
    	return EXIT_FAILURE;
  	}
  ...
  if (fileGradientImage3D.length() > 0) {
	...
	// f2 results
  if (fileQImage3D.length() > 0) {

		// Construct the output moving image and copy in the voxel values from the image vector
  	OutputImagePointer outputQImage = OutputImageType::New();
  	...
  	outputQImage->SetRegions( 	outputRegion );
  	outputQImage->SetOrigin( 	  origin3D );
  	outputQImage->SetSpacing( 	spacing3D );

  	outputQImage->Allocate();

		unsigned int iVoxel = 0; 
  	OutputImageConstIteratorType outputQImageIter( outputQImage, outputQImage->GetLargestPossibleRegion() );
		for ( outputQImageIter.GoToBegin(); !outputQImageIter.IsAtEnd(); ++outputQImageIter )
		{			
			outputQImage->SetPixel( outputQImageIter.GetIndex(), ((float) gradientImage.get(iVoxel)) );
			iVoxel++;
		}

  	// Then write the image

  	OutputImageWriterType::Pointer outputQImageWriter = OutputImageWriterType::New();

  	outputQImageWriter->SetFileName(  fileQImage3D );
  	outputQImageWriter->SetInput( 		outputQImage );

  	try {
    	std::cout << "Writing moving image output to file: " + fileQImage3D;
    	outputQImageWriter->Update();
  	}
  	catch( itk::ExceptionObject & err ) {
    	std::cerr << "ERROR: Failed to write moving image output to file: " << fileQImage3D << "; " << err << endl;
    	return EXIT_FAILURE;
  	}
    ...
    case OPTIMIZER_CONJUGATE_GRADIENT_MAXITER:
      {
        ...
        OptimizerType::Pointer optimizer = OptimizerType::New();
				...
				itk::ConjugateGradientMaxIterOptimizer::ScalesType scales( totalSize3D + parameters.Size() ); 
    ...
    case OPTIMIZER_REGULAR_STEP_GRADIENT_DESCENT:
      {
        ...
        OptimizerType::Pointer optimizer = OptimizerType::New();
				...
				itk::RegularStepGradientDescentOptimizer::ScalesType scales( totalSize3D + parameters.Size() );

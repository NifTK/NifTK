...
void Usage(char *exec)
{
  ...
  std::cout << "  " << std::endl
    ...
    << "    -iga  <doublex12>                 Input the 12 transformation parameters as the initial guess directly" << std::endl << std::endl

		<< "    -ifl  <double>                    The Finite Difference Method (FDM) lambda value [1000000]" << std::endl
		<< "    -ifd  <double>                    The Finite Difference Method (FDM) difference value [0.0001]" << std::endl
		<< "    -ife  <double>                    The Finite Difference Method (FDM) difference epsilon value [0.0000001]" << std::endl << std::endl

    << "    -tr                               Turn the trace on/off for the LBFGS optimiser" << std::endl
    ...
    << "           3    Conjugate gradient." << std::endl << std::endl
		...
		<< "    -iss  <double><doublex12>         Set two scales for the voxel intensities and  transformation parameters [1.0]" << std::endl
		<< "    -igt  <double>                    Specify the gradient convergence tolerance for LBFGS optimiser [1e-5]" << std::endl
		<< "    -ila  <double>                    Specify the line search accuracy for LBFGS optimiser (0.1-0.9) [0.9]" << std::endl
		<< "    -ist  <double>                    Specify the optimisation step length for LBFGS optimiser [1]" << std::endl << std::endl
...
int main(int argc, char** argv)
{
  ...
  typedef AffineTransformerType::EulerAffineTransformType 																				EulerAffineTransformType;
	typedef EulerAffineTransformType::Pointer 																											EulerAffineTransformPointer;
  ...
  bool flgGE_6000 					= false;			// Use the GE 6000 15 projection geometry
	...
	double lambdaValue				= 10000000;   // FDM lambda value
	double diffValue 					= 0.0001;     // FDM difference value
  double epsilonValue 			= 0.0000001;  // FDM difference epsilon value

	bool flgScaleSetted 			= false;
	bool flgGradTolSetted 		= false;
	bool flgLineSearchSetted 	= false;
	bool flgStepLengthSetted 	= false;
  double gradTolerance			= 1e-5;				// The gradient convergence tolerance for LBFGS optimiser
	double scaleIntensity 		= 1.0;     		// The optimiser scale for the voxel intensities
	double lineSearchAccuracy = 0.9;     		// The line search accuracy of LBFGS optimiser
	double optStepLength 			= 1.0;     		// The optimisation step length for LBFGS optimiser
  ...
  EulerAffineTransformType::ParametersType parameters(12);
	...
	parameters[2] = 1.;  	// Translation along the 'z' axis

	parameters[3] = 5.;  // Roation along the 'x' axis
	parameters[4] = 5.;  // Roation along the 'x' axis
	parameters[5] = 5.;  // Roation along the 'x' axis
  ...
  for(int i=1; i < argc; i++){
    if(strcmp(argv[i], "-help")==0 || strcmp(argv[i], "-Help")==0 || strcmp(argv[i], "-HELP")==0 
      ...
      return -1;
			...
			flgTrace = true;
      cout << "Trace for LBFGS optimiser is on" << endl;
    }
      ...
      std::cout << "Set -iga="
		...
		// Set the FDM lambda value (optional)

    else if(strcmp(argv[i], "-ifl") == 0) {
      lambdaValue = atof(argv[++i]);
      std::cout << "Set -ifl=" << niftk::ConvertToString(lambdaValue);
    }
      ...
      std::cout << "Set -ifd=" << niftk::ConvertToString(diffValue);
		...
		// Set the FDM epsilon value (optional)

    else if(strcmp(argv[i], "-ife") == 0) {
      epsilonValue = atof(argv[++i]);
      std::cout << "Set -ife=" << niftk::ConvertToString(epsilonValue);
    }
      ...
      std::cout << string("Set -iss=")
		...
		// Set the gradient convergence tolerance; LBFGS optimiser only (optional)

    else if(strcmp(argv[i], "-igt") == 0) {
      gradTolerance = atof(argv[++i]);
			flgGradTolSetted = true;
      std::cout << "Set -igt=" << niftk::ConvertToString(gradTolerance);
    }

		// Set the optimiser line search accuracy; LBFGS optimiser only (optional)

    else if(strcmp(argv[i], "-ila") == 0) {
      lineSearchAccuracy = atof(argv[++i]);
			flgLineSearchSetted = true;
      std::cout << "Set -ila=" << niftk::ConvertToString(lineSearchAccuracy);
    }

		// Set the optimisation step length; LBFGS optimiser only (optional)

    else if(strcmp(argv[i], "-ist") == 0) {
      optStepLength = atof(argv[++i]);
			flgStepLengthSetted = true;
      std::cout << "Set -ist=" << niftk::ConvertToString(optStepLength);
    }
  ...
  inImageEstimation->Allocate();
	inImageEstimation->FillBuffer( 0.1 );
  ...
  matrixBasedSimulReconRegnMetric->SetInputVolume( inImageEstimation );
	...
	matrixBasedSimulReconRegnMetric->SetInputVolumeVector( initialEstimateImageVector);
  ...
  matrixBasedSimulReconRegnMetric->SetInputTempProjections( outImage );
	...
	// Set the 3D reconstruction estimate input volume as a vector form
	matrixBasedSimulReconRegnMetric->SetParameterVector( initialParametersVector );

  /// Set the Finite Difference Method (FDM) lambda value, difference value, and epsilon value
  matrixBasedSimulReconRegnMetric->SetEulerTransformFDMLambda( lambdaValue );
  matrixBasedSimulReconRegnMetric->SetEulerTransformFDMDifference( diffValue );
  matrixBasedSimulReconRegnMetric->SetEulerTransformFDMDifferenceEpsilon( epsilonValue );
  ...
  MatrixBasedSimulReconRegnMethodType::Pointer imReconstructor = MatrixBasedSimulReconRegnMethodType::New();
	imReconstructor->SetReconEstimate(inImageEstimation);

	EulerAffineTransformPointer initialParametersPointer = EulerAffineTransformType::New();
  initialParametersPointer->SetParameters(parametersInitialGuess);
	imReconstructor->SetAffineEstimate(initialParametersPointer);
    ...
    case OPTIMIZER_LIMITED_MEMORY_BFGS:
      {
        ...
        if (flgGradTolSetted)
          optimizer->SetGradientConvergenceTolerance(gradTolerance);

        if (flgLineSearchSetted)
          optimizer->SetLineSearchAccuracy(lineSearchAccuracy);

        if (flgStepLengthSetted)
          optimizer->SetDefaultStepLength(optStepLength);

        std::cout << "LBFGS Maximum number of iterations set to: " << niftk::ConvertToString((int) nIterations);
        std::cout << "LBFGS gradient tolerance set to: " << niftk::ConvertToString((double) gradTolerance);
        std::cout << "LBFGS line search accuracy set to: " << niftk::ConvertToString((double) lineSearchAccuracy);
        std::cout << "LBFGS step length set to: " << niftk::ConvertToString((double) optStepLength);

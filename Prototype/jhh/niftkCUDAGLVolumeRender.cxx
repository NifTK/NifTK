...
struct niftk::CommandLineArgumentDescription clArgList[] = {
  ...
  {OPT_DONE, NULL, NULL, 
   "Program to volume render a 3D image.\n"
  }
};
...
#if 0
typedef unsigned char VolumeType;
#else
...
main( int argc, char** argv) 
{
  ...
  InputImageType::Pointer inImage3D = imageReader->GetOutput();

  const InputImageType::SpacingType& sp = inImage3D->GetSpacing();

  volumeSize.width  = sp[0];
  volumeSize.height = sp[1];
  volumeSize.depth  = sp[2];

  void *h_volume = inImage3D->GetBufferPointer();

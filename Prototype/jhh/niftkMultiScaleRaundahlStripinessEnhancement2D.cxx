...
struct niftk::CommandLineArgumentDescription clArgList[] = {
  ...
  {OPT_SWITCH, "suppress", 0, "Scale the stripiness measure by the magnitude of the largest absolute eigenvalue."},
  ...
  {OPT_DONE, NULL, NULL, 
   "Program to enhance 'stripey' features as per Raundahl et al "
...
int main( int argc, char *argv[] )
{
  ...
  CommandLineOptions.GetArgument(O_SUPPRESS, flgSuppress);
  ...
  // Create the stripiness enhancement filter
  // ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

  typedef itk::HessianToRaundahlStripinessMeasureImageFilter< HessianImageType,OutputImageType > StripinessFilterType;

  StripinessFilterType::Pointer stripinessFilter = StripinessFilterType::New();

  if (flgSuppress)
    stripinessFilter->SetScaleStripinessMeasure( true );
  else
    stripinessFilter->SetScaleStripinessMeasure( false );

  if ( flgDark )
    stripinessFilter->SetBrightObject( false );
  else
    stripinessFilter->SetBrightObject( true );
  ...
  multiScaleEnhancementFilter->SetHessianToMeasureFilter( stripinessFilter );
  ...
  try
  {
    std::cout << "Executing MultiScaleHessianBasedMeasureImageFilter.";

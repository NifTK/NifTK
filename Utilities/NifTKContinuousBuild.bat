...
@echo Running CMake....
@cd /d %BUILD_BIN%
call "%CMAKE_LOCATION%\cmake.exe" -DCMAKE_BUILD_TYPE=%BCONF% -DEP_BASE:PATH=%EXT_PROJ_ROOT% -DEP_DIRECTORY_PER_VERSION:BOOL=TRUE -DEP_ALWAYS_USE_INSTALL_DIR:BOOL=TRUE -DDESIRED_QT_VERSION:STRING=4 -DOPENCV_WITH_FFMPEG=ON -DNIFTK_BUILD_ALL_APPS=ON -DNIFTK_USE_CUDA=OFF -DNIFTK_USE_GIT_PROTOCOL=ON -DBUILD_TESTING=ON -DBUILD_COMMAND_LINE_PROGRAMS=ON -DBUILD_COMMAND_LINE_SCRIPTS=ON -DNIFTK_GENERATE_DOXYGEN_HELP=ON -DNIFTYLINK_CHECK_COVERAGE=ON -G "%CMAKE_GENERATOR%" "%BUILD_SRC%"

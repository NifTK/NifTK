...
::Execute()
{
  ...
  if (listOfSlicesToProcess.size() > 0)
  {
    ...
    for (unsigned int i = 0; i < listOfSlicesToProcess.size(); i++)
    {
      ...
      typename PointSetType::Pointer propagatedContours = PointSetType::New();
      if (m_Contours.IsNotNull())
      {
        this->PropagatePointList(axis, currentRegion, m_Contours, propagatedContours);
      
        // Render all contours into image.
        for (unsigned long int j = 0; j < propagatedContours->GetNumberOfPoints(); j++)
        {
          m_DestinationImage->TransformPhysicalPointToIndex(propagatedContours->GetPoints()->GetElement(j), voxelIndex);
          contoursImage->SetPixel(voxelIndex, foreground);  
        }      
      }

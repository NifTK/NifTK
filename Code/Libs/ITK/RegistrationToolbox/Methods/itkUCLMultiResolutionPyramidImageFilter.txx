/*=========================================================================
  ...
  Module:    $RCSfile: itkUCLMultiResolutionPyramidImageFilter.txx,v $
  Language:  C++
...
=========================================================================*/
#ifndef __itkUCLMultiResolutionPyramidImageFilter_txx
...
UCLMultiResolutionPyramidImageFilter<TInputImage, TOutputImage, TScheduleElement>
  ...
  unsigned int num )
{
  ...
  TScheduleElement startfactor = 1;
  startfactor = startfactor * static_cast<TScheduleElement>(niftk::Round(pow((double)2.0, (double)(m_NumberOfLevels - 1 ))));
...
void
UCLMultiResolutionPyramidImageFilter<TInputImage, TOutputImage, TScheduleElement>
::SetStartingShrinkFactors(TScheduleElement factor )
{

  TScheduleElement array[ImageDimension];
  ...
  const ScheduleType& schedule )
{
  ...
  if( schedule.rows() != m_NumberOfLevels ||
    ...
    {
    niftkitkDebugMacro(<< "Schedule has wrong dimensions" );
    return;
    }
...
{
  std::cout << "typeid(TScheduleElement)=" << typeid(TScheduleElement).name() << std::endl; 
  std::cout << "typeid(unsigned int)=" << typeid(unsigned int).name() << std::endl; 
  if (typeid(TScheduleElement) != typeid(unsigned int))
  {
    std::cout << "TScheduleElement is not unsigned int" << std::endl; 
    return false; 
  }
  std::cout << "TScheduleElement is unsigned int" << std::endl; 
  ...
  for( ilevel = 0; ilevel < schedule.rows() - 1; ilevel++ )
    ...
    for( idim = 0; idim < schedule.columns(); idim++ )
      ...
      if( schedule[ilevel][idim] == 0 )
        ...
        unsigned int schedule_ilevel_idim = static_cast<unsigned int>(schedule[ilevel][idim]); 
        unsigned int schedule_ilevel_1_idim = static_cast<unsigned int>(schedule[ilevel+1][idim]);
      if( ( schedule_ilevel_idim % schedule_ilevel_1_idim ) > 0 )
        {
        return false;
        }
      }
  ...
  std::cout << "IsScheduleDownwardDivisible=true" << std::endl; 
  return true; 
}
...
::GenerateData()
{
  ...
  for( ilevel = 0; ilevel < m_NumberOfLevels; ilevel++ )
    ...
    for( idim = 0; idim < ImageDimension; idim++ )
      ...
      shrinkFactors[idim] = static_cast<unsigned int>(m_Schedule[ilevel][idim]);
...
::GenerateOutputInformation()
{
  ...
  if ( !inputPtr  )
    {
    niftkitkErrorMacro( << "Input has not been set" );
    }
...
::GenerateOutputRequestedRegion(DataObject * refOutput )
{
  ...
  if( !ptr )
    {
    niftkitkErrorMacro( << "Could not cast refOutput to TOutputImage*." );
    }
  ...
  if ( ptr->GetRequestedRegion() == ptr->GetLargestPossibleRegion() )
    ...
    for( idim = 0; idim < TOutputImage::ImageDimension; idim++ )
      {
      TScheduleElement factor = m_Schedule[refLevel][idim];

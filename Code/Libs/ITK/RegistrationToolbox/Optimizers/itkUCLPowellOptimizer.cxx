...
{
  ParametersType coord(m_SpaceDimension); 
  for(unsigned int i=0; i<m_SpaceDimension; i++)
    {
    tempCoord[i] = this->m_LineOrigin[i] + x * this->m_LineDirection[i];
    coord[i] = tempCoord[i]/this->GetScales()[i]; 
    }

  niftkitkDebugMacro("x = " << x );
  double val;
  try
    {
    //val = (this->m_CostFunction->GetValue(tempCoord));
    //std::cout << "coord=" << coord << ", x=" << x << std::endl; 
    val = (this->m_CostFunction->GetValue(coord));
    if (vnl_math_isnan(val))
      {
              ...
              ParametersType & tempCoord)
{
  ...
  *f3 = this->GetLineValue( *x3, tempCoord );
  ...
  niftkitkDebugMacro("Initial: " << *x1 << ", " << *x2 << ", " << *x3 );
  //
  ...
  for (m_CurrentLineIteration = 0;
    ...
    if (vcl_fabs(x-middle_range) <= (tolerance2 - 0.5*(b-a))
      ...
      *extVal = functionValueOfX;
      ...
      niftkitkDebugMacro("x = " << *extX );
      niftkitkDebugMacro("val = " << *extVal );
      niftkitkDebugMacro("return 1" );
...
::StartOptimization()
{
  ...
  UCLPowellOptimizer::ParametersType p(m_SpaceDimension);
  UCLPowellOptimizer::ParametersType pt(m_SpaceDimension);
  UCLPowellOptimizer::ParametersType ptt(m_SpaceDimension);
  UCLPowellOptimizer::ParametersType lastP(m_SpaceDimension);
  p = this->GetInitialPosition();
  // all the dimensions have the same scales. 
  for (unsigned int i = 0; i < m_SpaceDimension; i++)
  {
  ...
  double ax, xx, bx;
  double fa, fx, fb;
  // MATT: Moved this inside the #if block to avoid compiler warning about unused variable initialStepLength.
  // I assumed someone else was using #if 0, or #if 1 for testing purposes, but the standard build was not using it.
  // Kelvin: thanks. 
  // double initialStepLength = this->m_StepLength;
  ...
  for (m_CurrentIteration = 0;
    ...
    if (2.0*vcl_fabs(fp-fx) <= m_ValueTolerance*(vcl_fabs(fp)+vcl_fabs(fx)))
      ...
      m_StopConditionDescription << "Cost function values at the current parameter ("
       ...
       // set the parameters in original scales.
       for (unsigned int i = 0; i < m_SpaceDimension; i++)
       {
          this->m_CurrentPosition[i] = this->m_CurrentPosition[i]/this->GetScales()[i];
       }
    ...
    std::cout << "Current parameter=" << p << ",value=" << fx << std::endl;
    ...
    std::cout << "Current parameter=" << p << ",value=" << fx << std::endl;
    std::cout << "Last parameter=" << lastP << ",value=" << fp << std::endl;
    double parameterChange = GetMeasureOfParameterChange(lastP, p); 
      
    std::cout << "parameterChange=" << parameterChange << std::endl; 
    if (parameterChange < m_ParameterTolerance)
      {
        m_StopConditionDescription << "Current parameter ("
                                  << p
                                  << ") and at the local extrema ("
                                  << lastP 
                                  << ") are within parameters Tolerance ("
                                  << m_ParameterTolerance << ")";
        std::cout << "Stopping within tolerance=" <<  m_ParameterTolerance << std::endl; 
        // set the parameters in original scales. 
        for (unsigned int j = 0; j < m_SpaceDimension; j++)
        {
          this->m_CurrentPosition[j] = this->m_CurrentPosition[j]/this->GetScales()[j];
        }
        this->InvokeEvent( EndEvent() );
        return;
      }
    ...
    if (parameterChange < 0.1*m_StepLength)
      {
        m_StepLength = std::max<double>(100.0*m_StepTolerance, m_StepLength/2.0); 
        m_StepLength = std::min<double>(m_StepLength, initialStepLength); 
        std::cout << "parameterChange=" <<  parameterChange << ", setting m_StepLength to " << m_StepLength << std::endl;
      }
    else if (parameterChange > 0.1*m_StepLength)
      {
        m_StepLength = std::max<double>(100.0*m_StepTolerance, m_StepLength*2.0);
        m_StepLength = std::min<double>(m_StepLength, initialStepLength);
        std::cout << "parameterChange=" <<  parameterChange << ", setting m_StepLength to " << m_StepLength << std::endl;

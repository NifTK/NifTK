...
<tr>
	<td align=center width="90%">
		<h1>BSI Report:</h1> PATIENT
	</td>
	<td align=right width="10%">
		<a href="http://cmic.cs.ucl.ac.uk" target="a"><img border src="http://cmic.cs.ucl.ac.uk/fileadmin/CMIC/images/CMIC-Logo_Web240.gif"></a>
	</td>
</tr>
</table>
<hr>
<p><b>Brain extraction results</b></p>
<p>These images show the tissue segmentation used to find the brain/non-brain boundary.</p>
<p><i>Baseline:</i> B_MASK <br><i>Volume:</i> VOLUME_1 mm^3<br><img border=0 src="FILE_B_MASK"></p>
<p><i>Follow-up:</i> F_MASK <br><i>Volume:</i> VOLUME_2 mm^3<br><img border=0 src="FILE_F_MASK"></p>
<hr>
<p><b>Brain extraction comparation between two time-points</b></p>
<p><img border=0 src="FILE_SEGMENTATION_COMPARATION"></p>
<hr>
<p><b>Baseline to follow-up registration results</b></p>
<p><img border=0 src="FILE_REGISTRATION_COMPARATION"></p>
<hr>
<p><b>XOR region where BSI is computed</b></p>
<p>Probability 0 <IMG SRC="orange.gif"> 1
<p><img border=0 src="FILE_XOR_REGION"></p>
<hr>
<p><b>Final brain edge movement image results</b></p>
<p>Atrophy <IMG SRC="blue.gif"> 0 <IMG SRC="orange.gif"> Growth</p>
<p><img border=0 src="FILE_BSI_RESULT"></p>
<hr>
<p><b>Estimated BSI and PBVC</b></p>
<p>BSI (ml)= BSI_VALUE<br>PBVC (%)= PBVC_VALUE<br>ATROPHY (%)= ATROPHY_VALUE<br>GROWTH (%)= GROWTH_VALUE</p>
<hr>
<p><b>Executed command line:</b></p>
<p><font face="courier" size="-1">COMMAND_LINE</font></p>
<hr>
<b>Referencing GBSI</b>

<p>Two-timepoint percentage brain volume change was estimated with GBSI [XXX], part of NifTK software tools 
[URL:<a href="http://cmic.cs.ucl.ac.uk/home/software/" target="a">http://cmic.cs.ucl.ac.uk/home/software/</a>].</p>

<font size="-1"><em>
<p>[X] ???. 
<BR>&nbsp;&nbsp;&nbsp;<a href="" target="a">???.</a>
<BR>&nbsp;&nbsp;&nbsp;???.
</p></em></font>

<br>
<b>Referencing BSI main method</b>
<font size="-1"><em>
...
<br>
<div align="center"><a href="http://cmic.cs.ucl.ac.uk" target="a">Centre for Medical Image Computing</a> - <a href="http://ucl.ac.uk" target="a">University College London</a></div>
<br>
</body>
</html>"""
...
def normalization(img,orientation): 
	"""
	###########################################################################
	# def normalization(img,orientation)
	# Function  : N3 normalization using Boyes et al. Neuroimage 2008 normalization process 
	# Param	    : img, input image
	...
	output_file=make_name_output_file(img,'_n3')
	
	if not os.path.isfile(output_file):
		execute_command_or_else_stop('niftkNormalization.py \
			-in '+img+' \
			-orient '+orientation+' \
...
def write_results(data):
	...
	###########################################################################
	"""
	report_file=os.path.abspath('report.kn-bsi')
...
def calculate_dw_BSI(method,tkn,tissue_to_remove_file):
	"""
	###########################################################################
	# def calculate_sw_BSI(method,tkn,tissue_to_remove_file)
	# Function  : We compute the single window BSI
...
def generate_report(patient,results,command):
	...
	html=html.replace('VOLUME_1',str(v1))
	html=html.replace('VOLUME_2',str(v2))
	html=html.replace('FILE_B_MASK',seg_1)
	html=html.replace('FILE_F_MASK',seg_2)
	html=html.replace('B_MASK',B_MASK)
	html=html.replace('F_MASK',F_MASK)
	...
	html=html.replace('BSI_VALUE',str(results[0]))
	html=html.replace('PBVC_VALUE',str(results[1]))
	html=html.replace('ATROPHY_VALUE',str(atrophy))
	html=html.replace('GROWTH_VALUE',str(growth))
   ...
   <parameters advanced="true">
	...
	<string-enumeration>
		...
		<description><![CDATA[Select a window:
			Single: single window BSI, it computes atrophy between CSF-WM, Freeborough et al. 1997 TMI 
			Double: double window BSI, it computes atrophy between CSF-WM and WM-GM as in Leung et al. Neuroimage 2010 
		]]></description>
...
Optional Arguments BSI computation:
  ...
  -pbsi			: computes BSI using probabilistic BSI by Ledig et al. MICCAI 2012 
  -pbsig		: computes BSI using probabilistic BSI with gamma=1 as in Ledig et al. MICCAI 2012 
  -sw			: single window BSI, CSF-WM, Freeborough et al. 1997 TMI [DEFAULT]
  -dw			: applies double window BSI, it computes BSI between CSF-WM and WM-GM as in Leung et al. Neuroimage 2010 
...
Optional Arguments:
  ...
  -norm			: applies Boyes et al. Neuroimage 2008 normalization process
  -orient <value>	: input images are in AXIAL (a), CORONAL (c) or SAGITTAL (s) orientation, it is needed for the normalization step
  -atlas_mask <filename>: is the mask atlas file (ex: ICBM-152/lin-1.0/icbm_avg_152_t1_tal_lin_mask.nii)
  -atlas_head <filename>: is to the atlas data file (ex: ICBM-152/lin-1.0/icbm_avg_152_t1_tal_lin.nii)
...
if norm:
	progress_xml(0.03,"Checking atlas files needed for intensity normalization.")
	check_program_exists('niftkNormalization.py')
	check_file_exists(ATLAS_MASK_IMAGE)
	check_file_exists(ATLAS_HEAD_IMAGE)
	if orientation == '':
...
if norm:
	...
	B_INPUT_IMAGE=normalization(B_INPUT_IMAGE,orientation)
	progress_xml(0.20,"Normalization - follow up image")
	F_INPUT_IMAGE=normalization(F_INPUT_IMAGE,orientation)
...
if single_window:
	results=calculate_sw_BSI(bsi_method,tkn,tissue_to_remove_file)
else:
	results=calculate_dw_BSI(bsi_method,tkn,tissue_to_remove_file)
...
# We download from internet the images that are used as legend
download_from_web("http://blakey.cs.ucl.ac.uk/blue.gif","blue.gif")
download_from_web("http://blakey.cs.ucl.ac.uk/orange.gif","orange.gif")
download_from_web("http://blakey.cs.ucl.ac.uk/bg.jpg","bg.jpg")

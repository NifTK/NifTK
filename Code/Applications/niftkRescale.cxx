...
{  
  ...
  filter->SetOutputMinimum(args.min);
  filter->SetOutputMaximum(args.max);
  

  std::cout << "Filtering with upper:" << niftk::ConvertToString(args.min)
  << ", lower:" << niftk::ConvertToString(args.max) << std::endl;
...
/**
 * \brief Takes image and does intensity rescaling in ITK style.
 */
int main(int argc, char** argv)
{
  ...
  for(int i=1; i < argc; i++){
    if(strcmp(argv[i], "-help")==0 || strcmp(argv[i], "-Help")==0 || strcmp(argv[i], "-HELP")==0 || strcmp(argv[i], "-h")==0 || strcmp(argv[i], "--h")==0){
      ...
      args.min=atoi(argv[++i]);
      std::cout << "Set -min=" << niftk::ConvertToString(args.min) << std::endl;
    }
    else if(strcmp(argv[i], "-max") == 0){
      args.max=atoi(argv[++i]);
      std::cout << "Set -max=" << niftk::ConvertToString(args.max) << std::endl;
    }
    else {

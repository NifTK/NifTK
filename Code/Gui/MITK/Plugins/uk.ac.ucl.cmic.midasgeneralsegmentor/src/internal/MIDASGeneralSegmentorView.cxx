...
void MIDASGeneralSegmentorView::CreateQtPartControl(QWidget *parent)
{
  ...
  m_BaseSegmentationViewControls->m_ToolSelectorWidget->m_ManualToolSelectionBox->SetDisplayedToolGroups("Seed Draw Poly");
  m_BaseSegmentationViewControls->m_ToolSelectorWidget->m_ManualToolSelectionBox->SetLayoutColumns(3);
  m_BaseSegmentationViewControls->m_ToolSelectorWidget->m_ManualToolSelectionBox->SetShowNames(true);
  m_BaseSegmentationViewControls->m_ToolSelectorWidget->m_ManualToolSelectionBox->SetGenerateAccelerators(false);
...
bool MIDASGeneralSegmentorView::SelectSeedTool()
{
  ...
  if (m_BaseSegmentationViewControls->m_ToolSelectorWidget->m_ManualToolSelectionBox->isEnabled())
  {
    mitk::ToolManager* toolManager = this->GetToolManager();
    int activeToolId = toolManager->GetActiveToolID();
          ...
          && beforeSliceNumber != afterSliceNumber)
      {
        ...
        try
        {
          ...
          else // so, "Retain Marks" is Off.
          {
            ...
            {
              niftk::OpThresholdApply::ProcessorPointer processor = niftk::OpThresholdApply::ProcessorType::New();
              niftk::OpThresholdApply *doApplyOp = new niftk::OpThresholdApply(niftk::OP_THRESHOLD_APPLY, true, outputRegion, processor, m_GeneralControls->m_ThresholdingCheckBox->isChecked());
              niftk::OpThresholdApply *undoApplyOp = new niftk::OpThresholdApply(niftk::OP_THRESHOLD_APPLY, false, outputRegion, processor, m_GeneralControls->m_ThresholdingCheckBox->isChecked());

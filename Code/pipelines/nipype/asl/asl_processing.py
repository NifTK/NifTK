...
def create_asl_processing_workflow(name='asl_processing',
    ...
    # Register the M0 onto the T1 map (only connected if the T1 map is provided)
    m0_to_t1_registration = pe.Node(niftyreg.RegAladin(aff_direct_flag = True), 
                                    name = 'm0_to_t1_registration')
    
    # As we're trying to estimate an affine transformation, and rotations and shears are confounded
    # easier just to optimise an affine directly for the ASL 
    asl_to_m0_registration = pe.MapNode(niftyreg.RegAladin(aff_direct_flag = True), 
                                        name = 'asl_to_m0_registration',
                                        iterfield=['flo_file'])

    # Resample the ASL in the M0 space
...
#    asl_model_fitting.inputs.dTinv2 = 50.5722
    ...
    # Output node
    output_node = pe.Node( interface=niu.IdentityInterface(fields=['cbf_file', 
                                                                   'error_file', 
                                                                   'syn_file',
                                                                   'm0_to_t1_transformation',
                                                                   'asl_to_m0_transformations']),
                           name="output_node" )
    ...
    if t1_provided == True:
        workflow.connect(input_node, 'in_t1', m0_to_t1_registration, 'ref_file')
        workflow.connect(select_m0map, 'out', m0_to_t1_registration, 'flo_file')
    ...
    workflow.connect(select_aslmaps, 'out_list', asl_to_m0_registration, 'flo_file')
    ...
    if t1_provided == True:
        workflow.connect(input_node, 'in_t1', asl_model_fitting, 't1map')
    ...
    workflow.connect(asl_model_fitting, 'error_file', output_node, 'error_file')
    workflow.connect(asl_model_fitting, 'syn_file', output_node, 'syn_file')
    workflow.connect(asl_to_m0_registration, 'aff_file', output_node, 'asl_to_m0_transformations')
    if t1_provided == True:
        workflow.connect(m0_to_t1_registration, 'aff_file', output_node, 'm0_to_t1_transformation')

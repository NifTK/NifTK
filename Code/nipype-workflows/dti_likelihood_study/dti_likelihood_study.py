...
def create_dti_reproducibility_study_workflow(name='create_dti_reproducibility_study'):
    ...
    tensor_2_dwi = pe.MapNode(interface = ttk.Tensor2Dwi(), 
                              name = 'tensor_2_dwi', 
                              iterfield = ['in_file'])
    ...
    tensor_2_dwi_2 = pe.MapNode(interface = ttk.Tensor2Dwi(), 
                                name = 'tensor_2_dwi_2', 
                                iterfield = ['in_file'])
    ...
    workflow.connect(tensor_resampling, 'res_file', tensor_2_dwi, 'in_file')
    workflow.connect(input_node, 'in_bvec_file',    tensor_2_dwi, 'in_bvec_file')
    workflow.connect(input_node, 'in_bval_file',    tensor_2_dwi, 'in_bval_file')
    workflow.connect(input_node, 'in_B0_file',      tensor_2_dwi, 'in_B0_file')
    workflow.connect(tensor_2_dwi, 'out_files', merge_dwis, 'in_files')
    ...
    workflow.connect(tensor_resampling_2, 'res_file', tensor_2_dwi_2, 'in_file')
    workflow.connect(input_node, 'in_bvec_file', tensor_2_dwi_2, 'in_bvec_file')
    workflow.connect(input_node, 'in_bval_file', tensor_2_dwi_2, 'in_bval_file')
    workflow.connect(input_node, 'in_B0_file', tensor_2_dwi_2, 'in_B0_file')
    workflow.connect(tensor_2_dwi_2, 'out_files', merge_dwis_2, 'in_files')
